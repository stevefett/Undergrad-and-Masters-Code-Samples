...
bool DONE = FALSE;
unsigned char globalBuffers[MAX_BUFFERS][BUFFER_SIZE];
int i;
...
int initialDelayValues[16]; //12

typedef void (*PFV)(void);

extern void TimerISREntry(void);
extern void PrinterISREntry(void);
extern void IRQ34ISREntry(void);
...
void InitializeInterrupts(){
    ...
    SetExceptionHandler(0x23, IRQ34ISREntry, idt_table);
    SetExceptionHandler(0x24, IRQ34ISREntry, idt_table);
    SetExceptionHandler(0x27, PrinterISREntry, idt_table);
...
void ReadDelayValues(){
    ...
    cons_printf("\nPrintSpooler DELAY time (sec): ");
    initialDelayValues[12] = 0;
    while(isdigit(number = cons_getchar())){
        initialDelayValues[12] = initialDelayValues[12]*10 + (number-'0');
    }
    
    cons_printf("\nProducer5 DELAY time (sec): ");
    initialDelayValues[13] = 0;
    while(isdigit(number = cons_getchar())){
        initialDelayValues[13] = initialDelayValues[13]*10 + (number-'0');
    }
    
    cons_printf("\nConsumer5 1 DELAY time (sec): ");
    initialDelayValues[14] = 0;
    while(isdigit(number = cons_getchar())){
        initialDelayValues[14] = initialDelayValues[14]*10 + (number-'0');
    }
    
    cons_printf("\nConsumer5 2 DELAY time (sec): ");
    initialDelayValues[15] = 0;
    while(isdigit(number = cons_getchar())){
        initialDelayValues[15] = initialDelayValues[15]*10 + (number-'0');
    }
}
...
void InitializeMailBoxes()
{
	...
	{
		if (i < 5)
			mailBoxes[i] = MailBoxCreate(i, DIRECTED);
		else
			mailBoxes[i] = MailBoxCreate(i, UNDIRECTED);
	}	

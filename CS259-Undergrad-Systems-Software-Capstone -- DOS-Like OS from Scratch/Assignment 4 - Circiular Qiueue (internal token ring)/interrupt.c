...
void SendMessageISR(TrapFramePtr frame)
{
	...
	if (IsEmpty(mbox->receivers))
	{
        ...
        AddEnvelopeToMailBox(mailBoxes[mailBoxID], message);
	}
	else
	{
...
void ReceiveMessageISR(TrapFramePtr frame)
{
	...
	else
	{
		if (mbox->type == UNDIRECTED)
		{
            ...
            free(RemoveHeadEnvelopeFromMailBox(mbox));
		}
		else
		{
			envelope = mbox->messages->head;
			...
			if (envelope != NULL)
			{
				...
				// Remove envelope from message list
                free(RemoveEnvelopeFromMailBox(mbox, envelope));
			}
			else
				TransferHeadPCB(pcbLists[RUNNING_LIST], mbox->receivers);
		}
...
void PrinterISR(){
    //Post to PRINTER_SEMAPHORE
    //Can't invoke another trap, so copied code from SemaphorePostISR
    semaphores[PRINTER_SEMAPHORE]->value += 1;
	if (semaphores[PRINTER_SEMAPHORE]->value <= 0 && !IsEmpty(semaphores[PRINTER_SEMAPHORE]->queue))	
	{
		TransferHeadPCB(semaphores[PRINTER_SEMAPHORE]->queue, pcbLists[READY_LIST]);
